...
static const sireflect_enum_desc_t key_reflection = {
    .name = "Key",
    .values = "{ A = 0, D = 1, W = 2, S = 3, Q = 4, Z = 5, E = 6, Left = 7, Right = 8, Up = 9, "
              "Down = 10, Space = 11, I = 12 }",
    .size = sizeof(Key),
    .align = _Alignof(Key),
};
...
static void update_keyboard(Keyboard *keyboard) {
    ...
    for (size_t i = 0; i < KeyCount; i++)
        keyboard->keys[i] = state[scancodes[i]];
}
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_EVENT_MOUSE_MOTION:
            ...
            pointer->pointer_type = event.motion.which == SDL_TOUCH_MOUSEID ? SiPointerTouch
                                    : event.motion.which == SDL_PEN_MOUSEID ? SiPointerPen
                                                                            : SiPointerMouse;
            pointer->x = event.motion.x;
            pointer->y = event.motion.y;
            pointer->delta_x += event.motion.xrel;
            pointer->delta_y += event.motion.yrel;
            pointer->buttons = event.motion.state;
            had_motion = true;
            break;
        ...
        case SDL_EVENT_MOUSE_BUTTON_UP: {
            ...
            pointer->pointer_type =
                event.button.which == SDL_TOUCH_MOUSEID ? SiPointerTouch : SiPointerMouse;
            ...
            if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                ...
                pointer->buttons &= ~bit;
                pointer->released |= bit;
            }
            push_edge((siinput_pointer_edge_t){
        ...
        case SDL_EVENT_MOUSE_WHEEL: {
            ...
            pointer->x = event.wheel.mouse_x;
            pointer->y = event.wheel.mouse_y;
            pointer->wheel_x += x;
            pointer->wheel_y += y;
            push_edge((siinput_pointer_edge_t){ event.wheel.timestamp,
                                                pointer->pointer_id,
                                                pointer->buttons,
                                                pointer->x,
                                                pointer->y,
        ...
        case SDL_EVENT_WINDOW_DESTROYED:
            if (pointer->buttons)
                push_edge((siinput_pointer_edge_t){ SDL_GetTicksNS(),
                                                    pointer->pointer_id,
                                                    pointer->buttons,
                                                    pointer->x,
                                                    pointer->y,
...
void sigpu_input_init(void) {
    ...
    begin_input_system = ecs_system(
        { .name = "BeginInput",
          .phase = EcsPreUpdate,
          .query.resources = { { .id = ecs_id(Pointer), .access = EcsInOut },
                               { .id = ecs_id(Keyboard), .access = EcsInOut } },
...
ecs_system_id_t sigpu_input_system(void) { return begin_input_system; }
uint16_t sigpu_input_resource_id(const char *name) {
    if (SDL_strcmp(name, "Keyboard") == 0)
        return ecs_id(Keyboard);
    if (SDL_strcmp(name, "Pointer") == 0)
        return ecs_id(Pointer);
    return 0;
}
...
uint16_t sigpu_input_modifiers(void) { return modifiers; }
