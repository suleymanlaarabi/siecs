...
In C++, the typed wrapper registers a component automatically on first use. Use
the `reflected(...)` declaration when its fields must be visible to the REST
explorer or JSON serialization:
