...
Components are copied into an inheriting entity by default. Configure a
component as shared when all instances should read the same base value:
...
The policy is fixed when the component is registered. Owned inheritance copies
...
The C++ methods accept several component types or values at once. The C API
expresses the same operation with the id-based calls:
