...
int main(void) {
    ...
    ecs_set(world, entity, Position, { .x = 10.0f, .y = 20.0f });
...
Rust uses the `siecs` crate; it does not copy `distr/siecs.c` or compile with `cc`.
...
fn main() {
    ...
    world.set(entity, Position { x: 10.0, y: 20.0 });
...
The same SIECS runtime can be consumed from C, C++, or Rust:
...
Use the published crate or the local path crate:
...
fn main() {
    ...
    let entity = world.entity();
    world.set(entity, Position { x: 10.0, y: 20.0 });
    world.get::<Position>(entity).unwrap().x += 1.0;
}
```
