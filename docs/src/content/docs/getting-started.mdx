...
On Windows, use `clang-cl` with the MSVC developer environment. SIECS uses C23
features that the Microsoft C compiler (`cl.exe`) does not currently parse;
`clang-cl` keeps the MSVC ABI and does not change the SIECS API or public
headers.
...
- Keep hot loops focused on iteration and avoid structural changes inside them.
