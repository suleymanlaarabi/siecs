...
void character_update(Character *character, float dt);
void character_take_damage(Character *character, int amount);
void character_render(const Character *character, Renderer *renderer);
```
...
float vec2_length_squared(const Vec2 *value);
```
...
```c
health_apply_damage(&health, amount);
quaternion_normalize(&quaternion);
asset_handle_is_valid(&asset_handle);
```
