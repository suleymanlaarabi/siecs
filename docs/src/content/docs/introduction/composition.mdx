...
```c
typedef struct { bool frozen; } MotionState;
```
...
```c
typedef enum { Idle, Walk, Run } AnimationMode;
typedef struct { AnimationMode mode; } AnimationState;
```
...
```c
typedef struct { ecs_entity_t entity; } Parent;
```
...
```c
if (object_is_renderable(object)) { /* ... */ }
if (object_can_move(object)) { /* ... */ }
```
