---
title: Relations
description: Model entity relationships with relation components, targets, ChildOf hierarchies, and explicit lifecycle rules.
---
...
<TabItem label="C">
```c
ecs_set(child, ChildOf, { .target = parent });
```
</TabItem>
<TabItem label="C++">
```cpp
...
ecs_set(entity, Targets, { .target = other_entity });
```
</TabItem>
<TabItem label="C++">
```cpp
