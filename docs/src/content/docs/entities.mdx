---
title: Entities
description: Create, validate, and destroy entities while understanding handle lifetime, liveness, and generation safety.
---
import { Tabs, TabItem } from '@astrojs/starlight/components';

Entities are `ecs_entity_t` handles created by a world.

<Tabs syncKey="language">
<TabItem label="C">
```c
ecs_entity_t entity = ecs_new();
```
</TabItem>
<TabItem label="C++">
```cpp
ecs::init();
auto entity = ecs::entity::create();
```
</TabItem>
</Tabs>

New entities start alive and have no user components.

When numeric entity order must follow allocation order, use
`ecs_new_no_reuse()` in C or `ecs::entity::create_no_reuse()` in C++. It always
allocates a new highest index and never consumes a freed index. This is
monotonic rather than contiguous: other entity allocations can still create
gaps, and normal `ecs_new()` calls can continue to reuse freed indices.

For abstract entities, bases, and inherited components, see
[Inheritance](../inheritance/).

## Liveness

Use `ecs_is_alive()` with entity handles created by the same world:

<Tabs syncKey="language">
<TabItem label="C">
```c
if (ecs_is_alive(entity)) {
    /* entity can be used */
}
```
</TabItem>
<TabItem label="C++">
```cpp
if (entity.is_alive()) {
    entity.disable();
}
```
</TabItem>
</Tabs>

`ecs_is_alive()` is not a general validator for arbitrary integers. Passing ids
that did not come from the world is not supported.

## Destroy Entities

<Tabs syncKey="language">
<TabItem label="C">
```c
ecs_kill(entity);
```
</TabItem>
<TabItem label="C++">
```cpp
entity.kill();
```
</TabItem>
</Tabs>

Killing an entity removes all of its components, runs remove hooks, removes the
entity from its current archetype table, and invalidates the handle.

After `ecs_kill()`, do not use the old handle with component access functions.

## Handle Layout

The public API treats `ecs_entity_t` as an opaque handle. Internally, SIECS packs
an entity index and generation into a 64-bit value so stale handles can be
detected.
