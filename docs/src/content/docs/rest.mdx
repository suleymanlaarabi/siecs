...
Scene uploads default to a `64 MiB` limit; configure `max_scene_bytes` in
`sirest_props_t` when a different limit is required.
...
| `GET /scene` | Returns the current scene as a native SIECS binary. |
| `POST /scene` | Loads a native SIECS binary scene into the current world. |
...
`GET /scene` responds with `application/octet-stream` and the exact native SIECS
scene bytes. `POST /scene` accepts those bytes with the same content type and
returns `204` when loading succeeds. Invalid or incompatible data returns `400`;
an upload over the configured limit returns `413`.

Loading has append semantics: `POST /scene` instantiates the saved entities in
the current world and does not clear or replace entities that already exist.
The binary is tied to the component and relation layout registered by the
...
The REST addon is a tooling feature for inspectors and editors. Do not expose it
on an untrusted network without authentication, routing restrictions, or a
