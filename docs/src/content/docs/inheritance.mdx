...
When a field is inherited, `ecs_field_kind(&it, index)` returns `EcsFieldShared`.
The pointer returned by `ecs_field()` points to the component stored on the
base, not to an array with one element per entity.
