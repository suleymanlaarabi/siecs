...
float seconds = delta_time->value;
```
</TabItem>
<TabItem label="C++">
```cpp
const auto &delta_time = ecs::resource<const DeltaTime>();
float seconds = delta_time.value;
```
</TabItem>
</Tabs>
...
The resource is declared in `query.resources` as access metadata. It is never a
