...
ecs::system("PausedDebug")
    .each([](const Position &position, const ecs::Disabled &) {
        (void)position;
    });
```
</TabItem>
...
<TabItem label="C++">
```cpp
// The C++ system builder currently uses the raw descriptor for `after` links.
ecs_system_id_t integrate = ecs_system({
    .name = "Integrate",
    .phase = EcsOnUpdate,
    .query = { .terms = { ecs_inout(ecs::component<Position>()),
                          ecs_in(ecs::component<Velocity>()) } },
    .callback = integrate_system,
});
```
</TabItem>
