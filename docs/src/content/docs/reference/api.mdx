...
(`component`, `relation`, `relation_source`, `component_requires`), resources
...
match is transitive through the inheritance chain. A query must contain at
least one term or an `is_a` target.
...
ecs_not(Component);
ecs_in_source(Component);
ecs_not_source(Component);
```
...
relation types with `ecs::relation<T>(flags)` and retrieve their internal source
id with `ecs::relation_source<T>()`.
...
Built-in events use `ecs::OnAdd`, `ecs::OnRemove`, and `ecs::OnSet`. Custom events
are represented by an empty C++ type:
...
Observer callbacks must be stateless and must receive at least one component
argument. They may also receive `ecs::entity` first and `ecs::res<T>` resource
