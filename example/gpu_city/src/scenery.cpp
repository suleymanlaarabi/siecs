...
void door(float x, float y, float z, float yaw = 0.0f, float width = 1.2f, float height = 2.1f) {
...
void building(float x, float z, int floors, Rgb facade, float yaw) {
    ...
    for (int floor = 0; floor < floors; ++floor) {
        ...
        if (floor == 0) {
            auto position = local(0.0f, 1.05f, front);
            door(position.x, position.y + PavementY, position.z, yaw);
            for (float wx : { -1.42f, 1.42f }) {
                auto p = local(wx, window_y, front);
                window(p.x, p.y + PavementY, p.z, yaw, 0.78f, 1.05f);
            }
        } else {
            for (float wx : { -1.15f, 1.15f }) {
                auto p = local(wx, window_y, front);
                window(p.x, p.y + PavementY, p.z, yaw);
            }
        }
        box(width + 0.12f,
            0.12f,
...
void tree(float x, float z, float y) {
...
void make_building(float x, float z, int floors, Rgb facade, float yaw) {
    building(x, z, floors, facade, yaw);
}

void make_park(float x, float z) { park(x, z); }
