...
class TrafficLight {
    static Position3d local(float x, float y, float z, float yaw, float lx, float ly, float lz) {
        const float c = std::cos(yaw), s = std::sin(yaw);
        return { x + lx * c + lz * s, y + ly, z - lx * s + lz * c };
    }

  public:
    static Rgb color_for(uint8_t role, bool lit) {
        constexpr Rgb colors[] = { { 255, 35, 25 }, { 255, 177, 18 }, { 35, 230, 75 } };
        Rgb color = colors[role];
        if (!lit)
            color = { uint8_t(color.r * 0.16f),
                      uint8_t(color.g * 0.16f),
                      uint8_t(color.b * 0.16f) };
        return color;
    }
  ...
  public:
    static void make(float x, float z, float yaw, bool green, ecs::entity node) {
...
class Road {
  public:
    static void section(float x, float z, bool horizontal) {
        geometry::box(
    ...
    static void crossing(float x, float z, bool horizontal) {
        for (int i = 0; i < 8; ++i) {
    ...
    static void intersection(float x, float z, int column, int row, bool roundabout) {
        ...
        if (roundabout) {
            ...
            TrafficLight::make(x - light, z - light, 3.14159265f, false, node);
            TrafficLight::make(x + light, z - light, 1.57079633f, true, node);
            TrafficLight::make(x - light, z + light, -1.57079633f, true, node);
            TrafficLight::make(x + light, z + light, 0.0f, false, node);
        }
    }
};
...
void make_road_section(float x, float z, bool horizontal) { Road::section(x, z, horizontal); }

void make_road_intersection(float x, float z, int column, int row, bool roundabout) {
    Road::intersection(x, z, column, row, roundabout);
}
