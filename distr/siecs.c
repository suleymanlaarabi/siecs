...
#if (SIECS_HAS_META) || (SIECS_HAS_REST)
...
#endif /* (SIECS_HAS_META) || (SIECS_HAS_REST) */
...
sijson_value_t ecs_rest_entity_children_json(ecs_entity_t entity) {
    ...
    const ecs_relation_record_t *childof = ecs_relation_record(ecs_rid(ChildOf));
    RelationSource *source = ecs_try_get_cid(entity, childof->component + 1);
    if (!source) {
        return children;
    }
...
sijson_value_t ecs_rest_entity_detail_json(ecs_entity_t entity) {
    ...
    ecs_entity_record_t *record = ecs_get_record(entity);
    ecs_table_t *table = ecs_get_table(record->table_id);
    for (uint32_t i = 0; i < table->type.component_count; i++) {
...
sihttp_response_t ecs_rest_get_entities(const sihttp_request_t *req) {
    ...
    ecs_query_entities(entity, ecs_optional(Abstract), ecs_optional(Disabled)) {
        if (!ecs_has_relation(entity, ChildOf)) {
