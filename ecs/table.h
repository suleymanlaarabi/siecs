...
typedef struct ecs_table_s {
    ecs_id_map_t add_edge; // maps component id to the table that has the component added or column index if the component is in the table
...
void ecs_table_init(ecs_table_t *table, ecs_type_t type, uint16_t tid, const struct ecs_component_index_s *component_index);
void ecs_table_fini(ecs_table_t *table);
uint32_t ecs_table_add_entity(ecs_table_t *table, ecs_entity_t entity);
// if the entity is not the last one, the last entity will be moved to the removed entity's position, and the moved entity will be returned
...
static inline uint16_t ecs_table_get_add_edge(const ecs_table_t *table, ecs_component_t component_id) {
...
static inline bool ecs_table_has(const ecs_table_t *table, ecs_component_t component_id) {
    ...
    return column_index < table->type.count && table->type.ids[column_index] == component_id;
}

static inline uint16_t ecs_table_get_column_index(ecs_table_t *table, ecs_component_t component_id) {
