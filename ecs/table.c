...
void ecs_table_init(ecs_table_t *table, ecs_type_t type, uint16_t, const ecs_component_index_t *component_index) {
    table->type = type;
...
static inline void ecs_table_grow(ecs_table_t *table) {
    ...
    for (uint16_t i = 0; i < table->type.count; i++) {
        if (table->cls[i].is_bitset) {
            ...
            memset((uint8_t*)table->cls[i].data + (old_word_count * sizeof(uint64_t)), 0, (new_word_count - old_word_count) * sizeof(uint64_t));
    ...
    if (row != last_row) {
        ...
        for (uint16_t i = 0; i < table->type.count; i++) {
            if (table->cls[i].is_bitset) {
                ...
                if (val) ecs_bitset_set(&bs, row);
                else ecs_bitset_unset(&bs, row);
            } else if (table->cls[i].size != 0) {
